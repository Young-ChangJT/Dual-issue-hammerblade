...
if (enable_vcore_profiling_p) begin
  // vanilla core profiler
   bind vanilla_core vanilla_core_profiler #(
    ...
    // explicit connections to renamed internal signals
    .clk_i(clk_i),
    .reset_i(reset_i),

    .pc_r(pc_r),
    .pc_n(pc_n),
    .if_pc(pc_r),               // IF stage PC currently just pc_r
    .id_pc(id_r.pc_plus4),      // ID stage
    .exe_pc(exe_r.pc_plus4),    // EXE stage

    .flush(icache_flush),
    .icache_miss(icache_miss),
    .icache_miss_in_pipe(icache_flush_r_lo),
    .stall_all(stall_all),
    .stall_id(stall_id),
    .stall_depend_long_op(stall_depend_long_op),
    .stall_depend_local_load(stall_depend_local_load),
    .stall_depend_imul(stall_depend_imul),
    .stall_bypass(stall_bypass),
    .stall_lr_aq(stall_lr_aq),
    .stall_fence(stall_fence),
    .stall_amo_aq(stall_amo_aq),
    .stall_amo_rl(stall_amo_rl),
    .stall_fdiv_busy(stall_fdiv_busy),
    .stall_idiv_busy(stall_idiv_busy),
    .stall_fcsr(stall_fcsr),
    .stall_remote_req(stall_remote_req),
    .stall_remote_credit(stall_remote_credit),

    .stall_barrier(stall_barrier),

    .stall_remote_ld_wb(stall_remote_ld_wb),
    .stall_ifetch_wait(stall_ifetch_wait),
    .stall_remote_flw_wb(stall_remote_flw_wb),

    .branch_mispredict(branch_mispredict),
    .jalr_mispredict(jalr_mispredict),
    .lsu_dmem_v_lo(lsu_dmem_v_lo),
    .lsu_remote_req_v_lo(lsu_remote_req_v_lo),
    .remote_req_o(remote_req_o),

    .rs1_val_to_exe(rs1_val_to_exe),
    .mem_addr_op2(mem_addr_op2),

    .int_sb_clear(int_sb_clear),
    .float_sb_clear(float_sb_clear),
    .int_sb_clear_id(int_sb_clear_id),
    .float_sb_clear_id(float_sb_clear_id),

    .id_r(id_r),
    .exe_r(exe_r),
    .fp_exe_ctrl_r(fp_exe_ctrl_r),

    .instruction(exe_r.instruction),
    .decode(exe_r.decode),
...
if (enable_vanilla_core_pc_histogram_p) begin
  ...
  vcore_pc_hist
    (
      ...
      .stall_remote_credit(stall_remote_credit),
      .stall_barrier(stall_barrier),
      .stall_icache_store(stall_icache_store),
      .stall_remote_ld_wb(stall_remote_ld_wb),
      .stall_ifetch_wait(stall_ifetch_wait),
      .stall_remote_flw_wb(stall_remote_flw_wb),
      .branch_mispredict(branch_mispredict),
