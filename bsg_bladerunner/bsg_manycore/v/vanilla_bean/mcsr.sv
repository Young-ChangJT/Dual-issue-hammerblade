  ...
  always_ff @ (posedge clk_i) begin
    if (reset_i) begin
      ...
      if (we_i && (addr_i == `RV32_CSR_BARCFG_ADDR) && (funct3_i == `RV32_CSRRW_FUN3)) begin
        ...
        //$display("[DBG][MCSR] barcfg write t=%0t src=%b dest=%0d", $time, data_i[0+:barrier_dirs_p], data_i[16+:barrier_lg_dirs_lp]);
  ...
  always_ff @ (posedge clk_i) begin
    if (reset_i) begin
      ...
      if (we_i && (addr_i == `RV32_CSR_BAR_PI_ADDR)) begin
        ...
        // synopsys translate_off
        //$display("[DBG][MCSR] barsend t=%0t pi_old=%0b pi_new=%0b po_in=%0b src=%b dest=%0d", $time, barrier_data_r, ~barrier_data_r, barrier_data_i, barrier_src_r, barrier_dest_r);
        // synopsys translate_on
      end
    end
  end
