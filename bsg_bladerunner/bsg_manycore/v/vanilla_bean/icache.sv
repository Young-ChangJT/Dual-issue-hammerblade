...
module icache
  ...
  (
    ...
    , output [pc_width_lp-1:0] pred_or_jump_addr1_o
    , output logic branch_predicted_taken1_o

    , output is_block_boundary_o
  ...
  // disabled assertion: unit test handles write sequencing explicitly
  /*
  ...
  */
  ...
  wire [icache_block_offset_width_lp-1:0]offset = pc_r[0+:icache_block_offset_width_lp];
  wire [icache_block_offset_width_lp-1:0]offset_next = offset + 1'b1;
  assign is_block_boundary_o = (&offset);
  assign instr_out = icache_data_lo.instr[offset];
  assign instr1_out = icache_data_lo.instr[offset_next];
  
  assign instr_o = instr_out;
  assign instr1_o = instr1_out;

  wire lower_sign_out = icache_data_lo.lower_sign[offset];
  wire lower_cout_out = icache_data_lo.lower_cout[offset];
  wire sel_pc    = ~(lower_sign_out ^ lower_cout_out); 
  wire sel_pc_p1 = (~lower_sign_out) & lower_cout_out; 

  wire lower_sign_out1 = icache_data_lo.lower_sign[offset_next];
  wire lower_cout_out1 = icache_data_lo.lower_cout[offset_next];
  wire sel_pc1    = ~(lower_sign_out1 ^ lower_cout_out1); 
  wire sel_pc_p1_1 = (~lower_sign_out1) & lower_cout_out1;
  ...
  logic [branch_pc_high_width_lp-1:0] branch_pc_high_out1;
  logic [jal_pc_high_width_lp-1:0] jal_pc_high_out1;
  ...
  always_comb begin
    if (sel_pc1) begin
      branch_pc_high_out1 = branch_pc_high;
      jal_pc_high_out1 = jal_pc_high;
    end
    else if (sel_pc_p1_1) begin
      branch_pc_high_out1 = branch_pc_high + 1'b1;
      jal_pc_high_out1 = jal_pc_high + 1'b1;
    end
    else begin 
      branch_pc_high_out1 = branch_pc_high - 1'b1;
      jal_pc_high_out1 = jal_pc_high - 1'b1;
  ...
  assign branch_pc1 = {branch_pc_high_out1, `RV32_Bimm_13extract(instr1_out)};
  assign jal_pc1 = {jal_pc_high_out1, `RV32_Jimm_21extract(instr1_out)};
  ...
  assign branch_predicted_taken1_o = lower_sign_out1;
